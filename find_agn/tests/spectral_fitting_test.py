...
@pytest.fixture()
def spec_object(ref_continuum_prepared):
    spectral_data = ref_continuum_prepared.get_spectral_data()
...
def test_fit_lowess_continuum(ref_continuum_raw):
    raw_spectral_data = ref_continuum_raw.get_spectral_data()
    continuum_fit = spectral_fitting.fit_lowess_continuum(raw_spectral_data, smoothing_frac=0.1)
...
def test_sanity_check_spectral_data(ref_continuum_raw):
    raw_spectral_data = ref_continuum_raw.get_spectral_data()
    # TODO could use some 'insane' test cases to fail against
    data = spectral_fitting.sanity_check_spectral_data(raw_spectral_data)


def test_fit_background(ref_continuum_raw):
    raw_spectral_data = ref_continuum_raw.get_spectral_data()
    data = spectral_fitting.fit_background(raw_spectral_data)
